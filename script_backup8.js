...
function spanishCoachInspect(answer){
  ...
  for(const [bad, good] of map){
    const reBad = new RegExp(`\\b${bad}\\b`,"gi");
    if(reBad.test(out)){
      out = out.replace(reBad, (m)=> applyCaseLike(m, good));
      fixes.push(`${bad} → ${good}`);
    }
  }

  if(/^oy\b/i.test(out)){
    out = out.replace(/^oy\b/i, (m)=> applyCaseLike(m,"voy"));
    fixes.push("oy → voy");
  }

  // agreement hint: paredes son blancos -> blancas
  let agreementNote = "";
  if(/paredes\s+son\s+blancos\b/i.test(out)){
    agreementNote = "Agreement: paredes son blancas";
    out = out.replace(/paredes\s+son\s+blancos\b/i, (m)=>{
      const cap = m[0]===m[0].toUpperCase();
      return cap ? "Las paredes son blancas" : "las paredes son blancas";
    });
    fixes.push("blancos → blancas");
  }

  return {
    fixed: tidySuggestion(out),
    notes: fixes,
    tag: fixes.length ? "Spelling/accents" : "",
    agreementNote
  };
}
...
if(text.includes("bedroom") || text.includes("habitación") || text.includes("dormitorio")){
  s = add(s, (lvl<=3) ? pickOne(["También tengo un armario.","También tengo pósters.","También tengo una lámpara."])
                      : pickOne(["Además, tengo un armario y un escritorio.","Además, tengo un escritorio y una lámpara.","Además, tengo pósters y una estantería."]));
} else if(text.includes("classroom") || text.includes("aula") || text.includes("clase")){
  s = add(s, (lvl<=3) ? pickOne(["También hay una pizarra.","También hay muchas mesas.","También hay ventanas."])
                      : pickOne(["Además, hay ventanas grandes y una pizarra.","Además, hay muchas mesas y una pizarra digital.","Además, hay pósters y mucha luz."]));
} else if(text.includes("school") || text.includes("coleg") || text.includes("escuela")){
  s = add(s, (lvl<=3) ? pickOne(["Además, hay un patio.","Además, hay una cafetería.","Además, hay una biblioteca."])
                      : pickOne(["Además, hay un patio y una biblioteca.","Además, hay una cafetería y un gimnasio.","Además, hay una biblioteca y laboratorios."]));
} else if(text.includes("routine") || text.includes("school day") || text.includes("día de colegio") || text.includes("día de clase") || text.includes("rutina")){
  s = add(s, (lvl<=3) ? pickOne(["Luego vuelvo a casa.","Después estudio un poco.","Luego hago los deberes."])
                      : pickOne(["Después vuelvo a casa y estudio un rato.","Luego hago los deberes y ayudo en casa.","Más tarde estudio y me acuesto temprano."]));
} else if(text.includes("family") || text.includes("familia")){
  s = add(s, (lvl<=3) ? pickOne(["Nos llevamos bien.","También somos muy divertidos.","Somos bastante unidos."])
                      : pickOne(["Nos llevamos muy bien y somos bastante unidos.","Además, nos gusta pasar tiempo juntos.","Además, somos unidos porque nos ayudamos mucho."]));
} else if(text.includes("town") || text.includes("ciudad") || text.includes("pueblo")){
  s = add(s, (lvl<=3) ? pickOne(["También hay parques.","También hay tiendas.","También hay un río."])
                      : pickOne(["Además, hay parques y muchos cafés.","Además, hay museos y muchos lugares bonitos.","Además, hay tiendas y un centro muy animado."]));
} else if(text.includes("teacher") || text.includes("profesor") || text.includes("maestro")){
  s = add(s, (lvl<=3) ? pickOne(["También explica muy bien.","También es muy paciente.","También es muy simpático."])
                      : pickOne(["Además, explica muy bien porque es paciente.","Además, es muy simpático y explica claro.","Además, ayuda mucho y tiene buen humor."]));
} else if(text.includes("friend") || text.includes("amigo")){
  s = add(s, (lvl<=3) ? pickOne(["También es muy amable.","También es muy divertido.","También es muy trabajador."])
                      : pickOne(["Además, es muy amable porque siempre me ayuda.","Además, es divertido y muy trabajador.","Además, siempre me ayuda y es buen amigo."]));
} else {
  s = add(s, (lvl<=3) ? pickOne(["También es muy interesante.","También es bastante chulo.","También es muy útil.","También es muy moderno."])
                      : pickOne(["Además, me gusta mucho porque es muy práctico.","Además, me encanta porque es muy interesante.","Además, es genial porque es cómodo.","Además, es bastante moderno y muy práctico."]));
}
  ...
  async function finishRound(){
    ...
    for(let i=0;i<PROMPTS_PER_ROUND;i++){ 
      ...
      const coach = (state.lang==="es") ? spanishCoachInspect(ans) : {fixed: tidySuggestion(ans), notes:[], tag:""};
      if(coach.tag){ coachCounts[coach.tag] = (coachCounts[coach.tag]||0)+1; }
...
} else if(ok && coach.tag){
  suggestion = coach.fixed;
} else if(ok){
...
const tip = (aiTip && String(aiTip).trim()) ? String(aiTip).trim()
           : (!ok ? `Coach target: fix this → ${reason}.` : "");
      ...
      items.push({ n:i+1, prompt:p.text, answer:ans||"—", ok, reason, suggestion, tip, why, coachTag: coach.tag||"", coachNotes: coach.notes||[], coachFixed: coach.fixed||"" });
    }


// -------- Coach focus (one goal at a time) --------
let coachFocusTag = "";
let coachFocusCount = 0;
{
  const entries = Object.entries(coachCounts).sort((a,b)=>b[1]-a[1]);
  if(entries.length && entries[0][1] >= 2){
    coachFocusTag = entries[0][0];
    coachFocusCount = entries[0][1];
  }
}

// Apply coach focus gently: only comment when it matches today's focus.
if(coachFocusTag){
  items.forEach(it=>{
    if(it.ok){
      if(it.coachTag === coachFocusTag && it.coachNotes && it.coachNotes.length){
        it.tip = `Coach focus: ${coachFocusTag} — ${it.coachNotes[0]}.`;
        // For spelling/accents focus, don't pile on more detail — just show the corrected version.
        if(coachFocusTag === "Spelling/accents" && it.coachFixed){
          it.suggestion = it.coachFixed;
        }
      } else {
        it.tip = "Nice — keep going.";
        if(coachFocusTag === "Spelling/accents"){
          // Avoid samey “add detail” when today is spelling: show a short model if needed.
          const model = pickModelAnswer({text: it.prompt}, "");
          if(model && !sameMeaning(model, it.answer)) it.suggestion = model;
        }
      }
    }
  });
} else {
  // No strong recurring pattern: default gentle target by level.
  items.forEach(it=>{
    if(it.ok && !it.tip){
      it.tip = (state.level>=4)
        ? "Coach target: add ONE extra detail + a reason (porque...)."
        : "Coach target: add ONE extra detail (también / además...).";
    }
  });
}
    ...
    state.mark = { items, wrong, scoreSec, passed, focus, coachFocus: coachFocusTag ? `${coachFocusTag} (${coachFocusCount})` : "" };
  ...
  function renderResults(){
    ...
    if(el.coachFocus) el.coachFocus.textContent = m.passed ? "✅ Passed — next level unlocked (in this theme)." : `${m.coachFocus ? "Coach focus today: "+m.coachFocus+". " : ""}Gym focus: ${m.focus}. ${state.gymRequired ? "Gym required (3+ wrong)." : "Try again."}`;
  ...
  function gymFocusType(){
    ...
    if(f.includes("spell") || f.includes("accent") || f.includes("orthograph") || f.includes("ortografía")) return "spelling";
...
function openGymFromResults(){
    ...
    const items = (state.mark.items||[]);
    const wrong = items.filter(it=>!it.ok);
    const byReason = wrong.filter(it=>String(it.reason||"")===focusReason);

    const pickWithAnswer = (arr)=> (arr||[]).find(it=>String(it.answer||"").trim().length>0);
    const refItem = pickWithAnswer(byReason)
                 || pickWithAnswer(wrong)
                 || byReason[0]
                 || wrong[0]
                 || items[0]
                 || {prompt:"", answer:""};
  ...
  if(el.wsPrompt) el.wsPrompt.textContent = (L==="es") ? "Pick the correctly spelled word." :
                                       (L==="fr") ? "Choisis le mot correctement orthographié." :
                                                    "Wähle das richtig geschriebene Wort.";
  const pairs = (L==="es") ? [
    ["habitacion","habitación"],
    ["espanol","español"],
    ["facil","fácil"],
  ...
  const bad = pair[0], good = pair[1];
  const opts = shuffle([
    good,
    bad,
    good.replace(/[áéíóúüñçêèàîôû]/g,"a"),
    bad.replace(/o/g,"ó")
  ]).slice(0,4);
  opts.forEach(o=>{
    const b=document.createElement("button");
    ...
    b.textContent=o;
    b.addEventListener("click", ()=>{
      const ok = norm(o)===norm(good);
      gymMark(ok, ok ? "Nice — that spelling/accent is correct." : "Try again: pick the correct spelling.");
    });
    el.wsChoices.appendChild(b);
  });
  if(el.wsHelp) el.wsHelp.textContent = (L==="es") ? `Target: accents/ñ where needed (e.g., ${good}).` : "Target: correct spelling/accents.";
}
      else if(type==="be"){
  ...
  const good = (L==="es") ? pickOne(["habitación","español","fácil","pequeño","paredes"]) :
               (L==="fr") ? pickOne(["école","très","garçon","intéressant"]) :
                            pickOne(["Schule","größer","für","weil"]);
      state.workshop.expectedWord = good;

  if(el.wsPrompt) el.wsPrompt.textContent = (L==="es") ? `Type the correct spelling: ${good}` :
                                           (L==="fr") ? `Tape l’orthographe correcte : ${good}` :
                                                        `Schreibe die korrekte Schreibweise: ${good}`;
  if(el.wsHelp) el.wsHelp.textContent = (L==="es") ? "Tip: accents and ñ matter here." : "Tip: correct spelling.";
    } else if(type==="be"){
      ...
      // Always show WHAT they're upgrading (prompt + their answer)
      ...
      const starter = (L==="es") ? "Use: También…, Además…, porque… / y…" :
                      (L==="fr") ? "Utilise: En plus…, Aussi…, parce que… / et…" :
                                   "Nutze: Außerdem…, auch…, weil… / und…";

      if(!baseAns){
        // Never show a blank target — fall back to the prompt
        if(el.wsPrompt) el.wsPrompt.textContent = (L==="es") ? `Write a NEW upgraded answer for: ${basePrompt || "the last prompt"}` :
                                                 (L==="fr") ? `Écris une NOUVELLE réponse améliorée pour : ${basePrompt || "le dernier prompt"}` :
                                                              `Schreibe eine NEUE verbesserte Antwort für: ${basePrompt || "die letzte Aufgabe"}`;
        if(el.wsHelp) el.wsHelp.textContent = `${starter}  •  Aim for 6+ words.`;
      } else {
        if(el.wsPrompt) el.wsPrompt.textContent = (L==="es") ? `Upgrade THIS answer (add ONE extra detail):\n"${baseAns}"` :
  ...
  const target = String(state.workshop.expectedWord||"").trim();
  ok = !!target && norm(val)===norm(target);
  msgOk = "Nice — correct spelling/accent.";
  msgNo = "Try again: match the target spelling exactly.";
    } else if(type==="be"){
