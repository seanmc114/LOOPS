...
(function () {
  ...
  // ----------------- Themes (Move 1.6) -----------------
  // Sequential 1–10 levels per theme. A theme earns 1 star per level passed (max 10).
  const THEMES = [
  ...
  function safeTheme(t){
    const id = String(t||"people").toLowerCase().trim();
    return THEMES.some(x=>x.id===id) ? id : "people";
  }
  ...
  function poolForLevel(level, theme) {
    const idx = Math.max(1, Math.min(10, level)) - 1;
    const base = PROMPT_BANK[idx] || PROMPT_BANK[0];
    const pool = Array.isArray(base) ? base : PROMPT_BANK[0];

    const th = safeTheme(theme);
    const textOf = (p)=> String(p && p.text || "").toLowerCase();

    const isMatch = (p)=>{
      const t = textOf(p);
      if (th === "people") return /(friend|teacher|person|family|sibling|mum|dad|mother|father|brother|sister|describe someone|your best)/.test(t);
      if (th === "school") return /(school|class|classroom|teacher|uniform|homework|exam|principal|rule|break|lunch|timetable|subject)/.test(t);
      if (th === "home") return /(house|home|room|bedroom|kitchen|living|garden|neighbourhood|street|flat|apartment)/.test(t);
      if (th === "free") return /(free time|weekend|hobby|music|sport|game|play|watch|film|series|holiday|travel|trip)/.test(t);
      if (th === "food") return /(food|eat|drink|restaurant|breakfast|lunch|dinner|café|menu|meal)/.test(t);
      return true;
    };

    const filtered = pool.filter(isMatch);
    // Ensure we can always draw 10 prompts
    return filtered.length >= PROMPTS_PER_ROUND ? filtered : pool;
  }

  // ----------------- Helpers -----------------
  function $(id) { return document.getElementById(id); }

  // Ensure Theme UI exists (we inject to avoid touching index.html for Move 1.6)
  function ensureThemeUI(){
    const home = $("screenHome");
    if(!home) return;

    // Insert Theme select field at top of the home grid
    let themeSel = $("themeSelect");
    if(!themeSel){
      ...
      if(grid){
        ...
        lab.setAttribute("for","themeSelect");
        ...
        field.appendChild(themeSel);
        grid.insertBefore(field, grid.firstChild);
      }
    }

    // Add a Theme progress box in the motivation area
    if(!$("themeStarsOut")){
      const mot = home.querySelector(".motivation");
      if(mot){
        const box = document.createElement("div");
        box.className = "motBox";
        const k = document.createElement("div");
        k.className = "motK";
        k.textContent = "Theme stars";
    ...
    // Add a Theme progress bar under the global progress bar
    if(!$("themeBarFill")){
      const global = home.querySelector(".globalBar");
      if(global && global.parentElement){
        ...
        t.id = "themeBarText";
        t.textContent = "Theme progress";

        const bar = document.createElement("div");
        bar.className = "globalBar";
        const fill = document.createElement("div");
        fill.className = "globalFill";
        fill.id = "themeBarFill";
        bar.appendChild(fill);

        wrap.appendChild(t);
        wrap.appendChild(bar);
        global.parentElement.appendChild(wrap);
      }
    }
  }
...
function toast(msg){
  ...
  function buildRound({ level, seed, lang, theme }) {
    const pool = poolForLevel(level, theme);
  ...
  const el = {
    ...
    pbOut: $("pbOut"),
    roundsOut: $("roundsOut"),
    themeStarsOut: $("themeStarsOut"),
    themeBarText: $("themeBarText"),
    themeBarFill: $("themeBarFill"),
  ...
  function pbKey(s) { return `LOOPS_PB_v4_${safeLang(s.lang)}_${safeTheme(s.theme)}_L${s.level}_${s.mode}`; }
  function roundsKey() { return "LOOPS_ROUNDS_v1"; }
  function doneKey() { return "LOOPS_DONE_v1"; } // json map of completed setup keys

    const TOTAL_STARS = THEMES.length * 10;

    // ----------------- Level unlocks (Move 1.6: per theme) -----------------
  function unlockKey(lang, theme){ return `LOOPS_UNLOCK_v2_${safeLang(lang)}_${safeTheme(theme)}`; }

  function loadUnlockedLevel(lang, theme){
    const raw = localStorage.getItem(unlockKey(lang, theme));
    const n = raw ? Number(raw) : 1;
    return Math.min(10, Math.max(1, isFinite(n) ? n : 1));
  }

  function saveUnlockedLevel(lang, theme, level){
    const n = Math.min(10, Math.max(1, Number(level)||1));
    localStorage.setItem(unlockKey(lang, theme), String(n));
  }

  function isLevelUnlocked(level, lang, theme){
    return Number(level) <= loadUnlockedLevel(lang, theme);
  }

  function starsForTheme(lang, theme){
    // 1 star per CLEARED level (sequential), max 10
    return Math.max(0, Math.min(10, loadUnlockedLevel(lang, theme) - 1));
  }

  function totalStars(lang){
    const L = safeLang(lang);
    return THEMES.reduce((sum,t)=> sum + starsForTheme(L, t.id), 0);
  }

  function lastThemeKey(lang){ return `LOOPS_LAST_THEME_v1_${safeLang(lang)}`; }
  function loadLastTheme(lang){
    const raw = localStorage.getItem(lastThemeKey(lang));
    return safeTheme(raw || "people");
  }
  function saveLastTheme(lang, theme){
    localStorage.setItem(lastThemeKey(lang), safeTheme(theme));
  }
  ...
  function syncGlobalProgress() {
    const stars = totalStars(state.lang);
    if (el.globalText) el.globalText.textContent = `${stars} / ${TOTAL_STARS} ★`;
    if (el.globalFill) el.globalFill.style.width = `${Math.min(100, (stars / TOTAL_STARS) * 100)}%`;
  }

  function syncThemeProgress(){
    const stars = starsForTheme(state.lang, state.theme);
    if (el.themeStarsOut) el.themeStarsOut.textContent = `${"★".repeat(stars)}${"☆".repeat(10-stars)}  (${stars}/10)`;
    if (el.themeBarFill) el.themeBarFill.style.width = `${stars * 10}%`;
    if (el.themeBarText){
      const th = THEMES.find(t=>t.id===state.theme);
      el.themeBarText.textContent = `${th ? th.label : "Theme"} progress: ${stars}/10`;
    }
  }
  ...
  function syncHints() {
    ...
    const unlocked = loadUnlockedLevel(state.lang, state.theme);
    if (el.unlockHint) {
      if (state.level < 10) {
  ...
  function buildLevelOptions() {
    ...
    // Clamp current level to unlocked range
    if (!isLevelUnlocked(state.level, state.lang, state.theme)) state.level = unlocked;
...
function syncWorkshopCta(){
  ...
  function startRound(seed) {
    resetRun();
    state.seed = seed;
    state.prompts = buildRound({ level: state.level, seed: state.seed, lang: state.lang, theme: state.theme });
  ...
  function renderAnswers(expanded) {
    ...
    state.prompts.forEach((p, i) => {
      ...
      // ----------------- COACH PANEL (always shown) -----------------
      const coach = document.createElement("div");
      coach.className = "answerCoach";

      // Use rubric (fast + fair) for pass/fail, even if AI returns no per-item list
      const check = (state.ai && state.ai.rubric && state.ai.rubric[i]) ? state.ai.rubric[i] : rubricCheck(state.answers[i], p, lang);
      const isComplete = !!(check && check.ok);

      const verdict = document.createElement("div");
      verdict.innerHTML = `<span class="tag">${isComplete ? "✅ Complete" : "❌ Incomplete"}</span>`;
      coach.appendChild(verdict);

      // Pull any AI-provided suggestion fields if they exist
      const aiItem = (state.ai && state.ai.ok && Array.isArray(state.ai.items) && state.ai.items[i]) ? state.ai.items[i] : null;

      const corr = aiItem ? (
        aiItem.correction || aiItem.best_correction || aiItem.bestCorrection || aiItem.model || aiItem.model_answer || aiItem.modelAnswer ||
      ...
      const tip = aiItem ? (aiItem.tip || aiItem.next_tip || aiItem.nextTip || aiItem.feedback || aiItem.kind_feedback || aiItem.kindFeedback || "") : "";
      const why = aiItem ? (aiItem.reason || aiItem.why || aiItem.rationale || aiItem.notes || "") : "";

      // If AI doesn't return a model answer, generate a local "upgrade" from THEIR answer first,
      // then fall back to a generic example by badge.
      let example = String(corr || "").trim();
      if (!example) example = smartExampleFromAnswer(txt, p, lang);
      if (!example) example = localExample(p, lang);

      if (example) {
        const c = document.createElement("div");
        c.textContent = `Example: ${example}`;
        coach.appendChild(c);
      }

      // Arcade-style requirement hint when incomplete
      const reqHint = (!isComplete && check) ? rubricHint(check.reason, lang) : "";
      if (reqHint) {
      ...
      // If AI gave a tip/why, show them (kept short)
      if (tip) {
        const t = document.createElement("div");
  ...
  function renderResults(firstRender) {
    ...
    if (el.workshopHint) {
      ...
      const topKey = top && top[0] ? top[0][0] : "";
      const topN = top && top[0] ? top[0][1] : 0;
      const focusLabel = topKey ? labelCog(topKey) : "your top cog";

      if (state.gymRequired && !state.workshop.cleared) {
        el.workshopHint.textContent = `3+ wrong → Gym required. Main focus: ${focusLabel}${topN ? ` (×${topN})` : ""}. Clear the goal, then try again.`;
      } else if (!passed) {
    ...
    if (firstRender) {
      ...
      if (passed && state.level === unlocked && unlocked < 10) {
        saveUnlockedLevel(state.lang, state.theme, unlocked + 1);
        buildLevelOptions();
...
el.homeBtn.addEventListener("click", () => {
  ...
  if (!passed) {
    toast("No worries — that level is still locked until you pass it. Try another theme for practice.");
  }
  // Option A: always allow home, but the failed level stays locked
  quitToHome();
});
...
if (el.wsHome) el.wsHome.addEventListener("click", () => {
  if (state.workshop.required && !state.workshop.cleared){
    toast("Training still needed here — but you can switch theme and come back stronger.");
  }
  quitToHome();
});
...
if (el.wsOverride) el.wsOverride.addEventListener("click", () => {
  ...
  // (Home selector listeners are wired in init() below)
...
function topCogsFromRound(){
  // Prefer rubric reasons (more truthful than badge), fall back to badge if needed.
  const counts = { accent:0, ser:0, structure:0, other:0 };

  function reasonToCog(reason, badge){
    if (reason === "missing_be") return "ser";
    if (reason === "missing_connector") return "structure";
    if (reason === "too_short" || reason === "blank") return "structure";
    // If we don't know, fall back to the badge (what the prompt was aiming at)
    if (badge === "ser" || badge === "structure" || badge === "accent") return badge;
    return "other";
  }

  for (let i = 0; i < PROMPTS_PER_ROUND; i++){
    if (!state.wrongMarks[i]) continue;
    const badge = (state.prompts[i] && state.prompts[i].badge) ? state.prompts[i].badge : "other";
    const reason = (state.ai && state.ai.rubric && state.ai.rubric[i] && state.ai.rubric[i].reason) ? state.ai.rubric[i].reason : "";
    const cog = reasonToCog(reason, badge);
    if (counts[cog] !== undefined) counts[cog]++; else counts.other++;
  }
...
function setWorkshopFeedback(msg, good){
  ...
  function rubricHint(reason, lang){
    ...
    if(reason === "blank") return "Write something (even one short sentence).";
    return "";
  }

  // Try to turn THEIR answer into a cleaner "possible good answer" (short, level-appropriate).
  // This guarantees students always see an attainable model, even if the AI returned nothing.
  function smartExampleFromAnswer(answerText, promptObj, lang){
    ...
    // Make comma fragments read like sentences
    s = s.replace(/\s*,\s*/g, ". ").replace(/\.+\s*\./g,". ").trim();

    if (lang === "es"){
      // common accent fixes (safe + motivating)
      s = s
        .replace(/\bespanol\b/gi,"español")
        .replace(/\bfrances\b/gi,"francés")
        .replace(/\bingles\b/gi,"inglés")
        .replace(/\baleman\b/gi,"alemán");

      // if they wrote "el de español" → coach-friendly upgrade
      s = s.replace(/^\s*el de\s+/i, "El profesor de ");
      s = s.replace(/^\s*la de\s+/i, "La profesora de ");

      // Ensure a be-form if badge expects it
      if (badge === "ser" && !hasBeForm(s, "es")){
        // If it starts with a noun phrase, insert "es"
        s = s.replace(/^(El [^ ]+[^.]*?)\s+(?!soy|eres|es|somos|sois|son)([a-záéíóúñ])/i, (m, a, b)=> `${a} es ${b}`);
      }
...
// ----------------- Init -----------------
  function init() {
    ...
    state.lang = "es";
    state.mode = "classic";
    state.theme = loadLastTheme(state.lang);
    state.level = 1;
    ...
    if (el.themeSelect) el.themeSelect.value = state.theme;
    buildLevelOptions();
    if (el.levelSelect) el.levelSelect.value = String(state.level);
    if (el.modeSelect) el.modeSelect.value = state.mode;

    // Wire up HOME selectors
    if (el.themeSelect){
      el.themeSelect.addEventListener("change", () => {
        state.theme = safeTheme(el.themeSelect.value);
        saveLastTheme(state.lang, state.theme);
        buildLevelOptions();
    ...
    if (el.levelSelect) {
      el.levelSelect.addEventListener("change", () => {
        const chosen = Math.min(10, Math.max(1, Number(el.levelSelect.value) || 1));
        if (!isLevelUnlocked(chosen, state.lang, state.theme)) {
          toast("Level locked — beat the target to unlock it.");
          buildLevelOptions();
          return;
        }
        state.level = chosen;
    ...
    if (el.langSelect) {
      el.langSelect.addEventListener("change", () => {
        state.lang = safeLang(String(el.langSelect.value || "es"));
        // Load last theme for this language
        state.theme = loadLastTheme(state.lang);
        if (el.themeSelect) el.themeSelect.value = state.theme;
        buildLevelOptions();
        syncPills(); syncHints(); syncHomeStats();
