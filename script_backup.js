...
// LOOPS: reset lock + workshop for this round
state.markLocked = false;
...
el.playAgainBtn.addEventListener("click", () => {
  if (!markingComplete()) { toast("Mark all 10 boxes first."); return; }
  if (!state.markLocked) { toast("Press “Lock feedback” to finish corrections."); return; }
  if (countWrong() > 0 && !state.workshop.cleared) { toast("Workshop time — fix your top cog first."); goToWorkshop(); return; }
  startSolo();
});
...
if (el.lockFeedbackBtn) el.lockFeedbackBtn.addEventListener("click", () => {
  if (!markingComplete()) { toast("Mark all 10 boxes first."); return; }
  state.markLocked = true;
  renderResults(false);
  toast("Feedback locked ✓");
});

if (el.workshopBtn) el.workshopBtn.addEventListener("click", () => {
  if (!markingComplete()) { toast("Mark all 10 boxes first."); return; }
  if (!state.markLocked) { toast("Press “Lock feedback” first."); return; }
  goToWorkshop();
});
...
if (el.wsOverride) el.wsOverride.addEventListener("click", () => {
  ...
  el.allCorrectBtn.addEventListener("click", () => {
    if (state.markLocked) { toast("Feedback is locked."); return; }
    state.wrongMarks = Array(PROMPTS_PER_ROUND).fill(false);
...
function pickGate(){
  // Vary gates. Keep them short + achievable.
  const pool = [
    { type:"streak", target:4 },
    { type:"repairs", target:6 },
    { type:"calibration", target:6, maxSlips:2 },
  ];
...
function goToWorkshop(){
  ...
  state.workshop.required = countWrong() > 0;
...
function renderWorkshopTop(){
  ...
  if (el.wsSubtitle){
    const wrong = countWrong();
    el.wsSubtitle.textContent = wrong > 0
      ? "Fix your top error cog, then unlock the exit."
      : "Perfect round — optional practice to level up.";
  }
}
...
function nextWorkshopItem(){
  ...
  if (el.wsHelp) el.wsHelp.textContent = "Any sentence is fine — just include the connector.";
}
...
function updateGateProgress(){
  ...
  if (el.wsMeterText){
    const slips = Math.max(0, s.attempts - s.correct);
    el.wsMeterText.textContent = `${Math.round(pct)}% • Correct ${s.correct} • Attempts ${s.attempts} • Streak ${s.streak} • Slips ${slips}`;
  }
...
function syncExitLock(){
  ...
  if (state.workshop.required && !state.workshop.cleared){
    el.wsExit.textContent = "Exit workshop 🔒";
