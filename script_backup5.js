...
function openGymFromResults(){
    ...
    if(el.wsCogs){ el.wsCogs.innerHTML=""; ["Main focus: "+focus,"Quick goal: "+state.workshop.gate.target+" in a row","Tip: small + clear"].forEach(t=>{ const d=document.createElement("div"); d.className="wsCog"; d.textContent=t; el.wsCogs.appendChild(d); }); }
  ...
  function nextGymDrill(){
    ...
    if(type==="connector"){
      ...
      if(el.wsPrompt) el.wsPrompt.textContent = (L==="es") ? "Type ONE upgraded answer (add one extra detail)." :
                                               (L==="fr") ? "Tape UNE réponse améliorée (un détail en plus)." :
                                                            "Schreibe EINE bessere Antwort (ein Detail mehr).";
      if(el.wsHelp) el.wsHelp.textContent = "Aim for 6+ words.";
    } else {
