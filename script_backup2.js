  ...
  function renderAnswers(expanded) {
    ...
    state.prompts.forEach((p, i) => {
      ...
      if (aiItem) {
        ...
        const isCorrect = (aiItem.is_correct ?? aiItem.isCorrect ?? aiItem.correct ?? false) === true;
        const verdict = document.createElement("div");
        verdict.innerHTML = `<span class="tag">${isCorrect ? "✅ Correct" : "❌ Needs work"}</span>`;
        coach.appendChild(verdict);

        const corr = aiItem.correction || aiItem.best_correction || aiItem.bestCorrection || aiItem.model || aiItem.suggested || aiItem.fix || "";
        const tip = aiItem.tip || aiItem.next_tip || aiItem.nextTip || aiItem.feedback || aiItem.kind_feedback || aiItem.kindFeedback || "";

        if (corr) {
          const c = document.createElement("div");
          c.textContent = `Suggested: ${corr}`;
          coach.appendChild(c);
        }
        if (tip) {
  ...
  function goToResults() {
    ...
    (async () => {
      try {
        ...
        for (let i = 0; i < PROMPTS_PER_ROUND; i++) {
          const it = items[i] || {};
          const isCorrect = (it.is_correct ?? it.isCorrect ?? it.correct ?? false) === true;
          state.wrongMarks[i] = blanks[i] ? true : !isCorrect;
        }
      } catch (err) {
        state.ai.ok = false;
        state.ai.error = (err && err.message) ? err.message : "Unknown error";

        // Fallback: mark only blanks wrong (lenient)
        for (let i = 0; i < PROMPTS_PER_ROUND; i++) {
