...
(function(){
  ...
  function gymTarget(level, wrong){
    const base = 2 + Math.min(5, Math.max(1, Number(level)||1) * 0.6);
    const sev = (wrong>=7)?2:(wrong>=5)?1:0;
    return Math.round(base + sev);
  }
  ...
  function buildPromptUI(){
    ...
    wrap.innerHTML = `
      ...
      <div class="inputRow">
        <input class="input mainInput" id="mainInput" autocomplete="off" spellcheck="false" placeholder="${LANGS[state.lang].placeholder}" />
    ...
    input.addEventListener("keydown", (e)=>{ if(e.key==="Enter"){ e.preventDefault(); handleNext(); } });
  ...
  function connectorPresent(s){ const x=norm(s); return /(\by\b|\bpero\b|\bporque\b|\bademas\b|\bentonces\b)/.test(x) || /(\bet\b|\bmais\b|\bparce\s+que\b|\bdonc\b)/.test(x) || /(\bund\b|\baber\b|\bweil\b|\bdeshalb\b)/.test(x); }
  ...
  function openGymFromResults(){
    const focus = state.mark.focus || "Detail";
    ...
    if(el.wsCogs){ el.wsCogs.innerHTML=""; ["Main focus: "+focus,"Add detail","Use a connector"].forEach(t=>{ const d=document.createElement("div"); d.className="wsCog"; d.textContent=t; el.wsCogs.appendChild(d); }); }
  ...
  function nextGymDrill(){
    ...
    if(variant==="choice"){
      ...
      const bad = (state.lang==="es") ? "Es bueno." : (state.lang==="fr") ? "C’est bien." : "Es ist gut.";
      ...
      if(el.wsPrompt) el.wsPrompt.textContent = (state.lang==="es") ? "Type ONE improved sentence using y/pero/porque." : (state.lang==="fr") ? "Tape UNE phrase avec et/mais/parce que." : "Schreibe EINEN Satz mit und/aber/weil.";
      if(el.wsHelp) el.wsHelp.textContent = "Example: … y … / … pero … / … porque …";
  ...
  function handleGymSubmit(){
    ...
    const ok = val.length >= 12 && connectorPresent(val);
    gymMark(ok, ok ? "Great — connector spotted!" : "Try again: add a connector.");
  }
